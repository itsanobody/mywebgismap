<template>
	<div class="XZQHComponent-pannel" v-show="this.$store.getters._getDefaultXZQHVisible">
		...
		<div class="XZQH-select-pannel">
			...
			<el-select v-model="provinceValue" @change="handleProvinceValueChange" size="mini" clearable placeholder="请选择省份">
			...
			<tbody>
				<tr v-for="item in cityAndCountyOptions" :key="item.label">
					<td style="min-width: 50px">
						<span class="city-item" :value="item.value" @click="handleItemClick(item.value, 'city')">{{ item.label }}</span>
					</td>
					<td>
						<span
							class="county-item"
	...
	methods: {
		...
		async getProvinceData() {
			...
			const queryTask = new QueryTask({
				url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/province6/FeatureServer/0'
			});
			let query = new Query();
		...
		async getCityAndCountyData(value) {
			...
			const [QueryTask, Query] = await loadModules(['esri/tasks/QueryTask', 'esri/tasks/support/Query'], options);
			const queryTask = new QueryTask({
				url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/city4326/FeatureServer/0'
			});
			let query = new Query();
			query.returnGeometry = false;
			query.outFields = ['*'];
				...
				Promise.all(
					currentCityData.map(async item2 => {
						...
						const queryTask2 = new QueryTask({
							url: 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/county4326_Code/FeatureServer/0'
						});
		...
		async handleItemClick(val, type) {
			...
			if (type === 'city') {
				...
				serverUrl = 'hhttps://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/city4326/FeatureServer/0';
			} else if (type === 'county') {
				code = val.toString().substring(0, 6);
				serverUrl = 'https://services3.arcgis.com/U26uBjSD32d7xvm2/arcgis/rest/services/county4326_Code/FeatureServer/0';
			}
			const [QueryTask, Query, Graphic] = await loadModules(['esri/tasks/QueryTask', 'esri/tasks/support/Query', 'esri/Graphic'], options);
			const queryTask = new QueryTask({
				url: serverUrl
			});
			let query = new Query();
